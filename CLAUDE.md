...
The sync runs **automatically every 24 hours** via a Cloudflare Cron Trigger (`wrangler.jsonc`
`triggers.crons` = `0 0 * * *` → `scheduled()` in `src/index.ts`). The schedule is set by
...
the value — currently `5`, a no-op against a 24h cron). Both cron and `POST /api/sync` call the
