...
The first rule processes INDmoney "Units allotted" body emails for `nm2580@gmail.com` and writes
them to the MF Transactions sheet without reading a PDF.
...
- **Parsers live in the `parsers` registry in `src/rules.ts`, keyed by name.** A parser takes the source text (PDF text or email body) and returns **`string[][]`** — zero or more rows, each aligned to that rule's `columns` — or `null`/empty for a parse failure (→ email skipped, not labeled, so it retries). One email can yield many rows (e.g. a multi-trade contract note); `/api/sync` dedups + inserts each row, then labels the email **once**. These are layout-dependent and the most fragile code — tune regexes using the `textPreview` from `/api/extract` against a real sample; do not change the pipeline. `indmoney-units-allotted` is `src/parse-indmoney-body.ts:parseIndmoneyBody` (body, one row); `nse-contract-note` is `pdf/parse-nse.ts:parseNseTrades` (PDF, multi-row, derives Date from the 17-digit Trade No prefix, maps B/S→Buy/Sell, Status always "Success"); `nse-contract-note-nit` reuses `parseNseTrades` but trims columns to 5 (no Status) for Nit's sheet; `invesco-processed-body` is `src/parse-invesco-body.ts:parseInvescoBody` (reads email body, returns single row, converts date to `D MMM 'YY` and amount to Indian Rupee format, maps and normalizes mutual fund name). Note that `parseNseTrades` standardizes common ETF/Stock names (like Motilal Oswal, UTI, Nippon, SBI) via the `cleanStockName` helper in `src/pdf/parse-nse.ts`.
...
- **The INDmoney mutual-fund source is the allotment email body, only for `nm2580@gmail.com`.** It matches the original sender `transactions@transactions.indmoney.com` and the subject prefix `Order Successful: Units allotted for your mutual fund buy`, then extracts Fund, Order date, Buy Amount, Units, and Allotted NAV. Units are rounded to 2dp to match existing rows.
...
- **INDmoney field mapping:** "Allotted NAV" → `NAV` (price/unit), "Units alloted/allotted" → `Units` (count).

## Config

- Secrets via `.dev.vars` (local) / `wrangler secret put` (prod): `GMAIL_CLIENT_ID`, `GMAIL_CLIENT_SECRET`, `GMAIL_REFRESH_TOKEN`, and one PDF password per distinct `passwordEnv` used in `rules.json` (currently `PDF_PASSWORD_NIT` and `PDF_PASSWORD_AR` for the two NSE recipients). The `Env` interface in `src/config.ts` is the source of truth — keep it in sync with both.
